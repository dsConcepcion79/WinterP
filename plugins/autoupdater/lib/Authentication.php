...
class AutoUpdater_Authentication
{
    ...
    public function validate($payload, $method)
    {
        ...
        if (!$received_timestamp || $received_timestamp < $timestamp) {
            AutoUpdater_Log::error(sprintf('Invalid timestamp, expected less than "%s" but received "%s". Received %s request to %s', $timestamp, $payload['wpe_timestamp'], strtoupper($method), AutoUpdater_Request::getCurrentUrl()));
