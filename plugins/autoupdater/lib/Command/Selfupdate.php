...
class AutoUpdater_Command_Selfupdate extends AutoUpdater_Command_Base
{
    ...
    public function __invoke($args, $assoc_args)
    {
        ...
        if (!$installer->update()) {
            WP_CLI::error(sprintf('Failed to run post-update proccess after updating to version %s. Try again!', $new_version));
            return;
        }

        WP_CLI::success(sprintf('Plugin has been successfuly update to version %s', $new_version));
    }
